...
#### For experiments with quantity variable manipulations, do an analysis a quantity analysis
  ...
  overOut <- ch.moralsGetAndPlotQuantDirOverlap(data.ovrlp, "IA1", "IB1", "overlap", "direction", c(1,10,40), filename = do.filename)
  #Then analyze the effects of quantity
  quantData <- ch.moralsQuantAnalyis(analysisReadyData.sn, "Item1", "Item2", c(1,10,40),"keyDef", respChoiceVal = c("Yes", "No"),"targetPresent", c(TRUE, FALSE), "overlapRound", params)
