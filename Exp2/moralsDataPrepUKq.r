...
itemAnalDat <- ch.moralsItemChoiceAnalysis(analysisReadyData.sn, "Item1", "Item2", "overlapRound", "dirOverlap","keyDef", respChoiceVal = c("Yes", "No"), params = params, saveFigures = T)
...
#### For experiments with quantity variable manipulations, do an analysis a quantity analysis
  ...
  quantData <- ch.moralsQuantAnalyis(analysisReadyData.sn, "Item1", "Item2", c(1,10,40),"keyDef", respChoiceVal = c("Yes", "No"),"targetPresent", c(TRUE, FALSE), "overlapRound", params = params, showLegend = FALSE)
